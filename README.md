# Hamburgerlar Radar - A Simple Harness for Arxiv Papers
...
**上下文/记忆（按飞书会话逻辑）**：会话键取自消息线程——飞书**话题内回复**共享 `thread_id`、
**引用回复**用 `root_id` 串联，二者都会**带上多轮历史**；群里**直接 @** 用消息自身 id，视为**新会话**（清空上下文）。
每会话保留最近约 6 轮对话，**持久化到 SQLite**（`conversation_state` 表，容器重启不失忆、多进程可共享）。
`last_arxiv_id`（指代"这篇"）与"待确认保存"也按会话隔离；会话 7 天未活动自动清理、待确认 10 分钟过期；
访问加锁保证飞书长连接多线程下的一致性。
